...
.onAttach <- function(libname, pkgname) {
  ...
  # We can't use version number since the relevant fix is not in an 
  # incremented package version number (still 0.6.3); we use the *Packaged* 
  # attribute as a proxy.
  ...
  if(isTRUE(should_update)) {
    packageStartupMessage("For best results, restart R session and update ",
                          "pander using devtools:: or remotes::",
